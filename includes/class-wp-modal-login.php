<?php
    ...
    class Tdp_frontend_WP_Modal_Login {
        ...
        public function ajax_login() {
            ...
            elseif ( isset( $_REQUEST['register'] ) ) {
                $user_data = array(
                    ...
                    'user_login' => sanitize_user( $_REQUEST['email'] ),
                ...
                $user_register = $this->register_new_user( $user_data['user_email'], $user_data['role'] );
         ...
         * @since 2.0
         */
        function register_new_user( $user_email, $role) {
            $errors = new WP_Error();
            $sanitized_user_login = apply_filters( 'user_registration_email', $user_email );
            ...
            // $user_id = wp_create_user( $user_email, $user_pass );
            ...
            // $user_type = 0;
            wp_new_user_notification( $user_id, $user_pass, $user_type = 0 );

            // $site_url = network_site_url( '/' );
            // $subject = sprintf( __('[%s] %s'), get_option('blogname'), $problem );
            // $message  = sprintf( __('Username: %s'), $user_email ) . "\r\n";
            // $message .= sprintf( __('Password: %s'), $user_pass ) . "\r\n\r\n";
            // $message .= sprintf( __('Go to: %s click on Sign in on the top right'), $site_url) . "\r\n\r\n";
        ...
        function retrieve_password( $user_data ) {
            ...
            if ( empty( $user_data ) ) {
                $errors->add( 'empty_username', __( 'Please enter a username or e-mail address.', 'tdp-fec' ) );
            } else if ( strpos( $user_data, '@' ) ) {
                $user_data = get_user_by( 'email', trim( $user_data ) );
                if ( empty( $user_data ) )
                    $errors->add( 'invalid_email', __( 'There is no user registered with that email address.', 'tdp-fec'  ) );
            } else {
            ...
            if ( ! $user_data ) {
                $errors->add( 'invalidcombo', __( 'Invalid username or e-mail.', 'tdp-fec' ) );
                return $errors;
            }
            ...
            <div id="login-box" class="login-popup">
                ...
                <?php if( ! $user_ID ) : ?>
                    <div class="section-container">
                        ...
                        <?php // Login Form ?>
                        <div id="login" class="wpml-content same-form" style="display:none;">
                            ...
                            if ( ( get_option( 'users_can_register' ) && ! is_multisite() ) || ( $multisite_reg == 'all' || $multisite_reg == 'blog' || $multisite_reg == 'user' ) ) : ?>
                                ...
                                <h2>Private party</h2>
                                    ...
                                    <form action="register" method="post" id="form" class="group" name="loginform">
                                        ...
                                        <!-- <p class="field-set" style="display: none;">
...
<!--                                        <p class="field-set">
                                            <label class="field-titles" for="role"><?php _e( 'Are you a dealer or private owner?', 'tdp-fec' ); ?></label>
                        ...
                        <div id="forgotten" class="wpml-content same-form" style="display:none;">
                            ...
                            <h2><?php _e( 'Lost Password?', 'tdp-fec' ); ?></h2>
                        ...
                        <div id="dealers" class="wpml-content same-form" style="display:none;">

                            <h2>DEALER SERVICES</h2>

                            <h3 style="color:#fff;"><span style="font-size: 350%; color: #ED6639;" class="icon-rocket"></span><br />
                                Need to move your inventory fast? Checkout out our <a href="<?php echo get_home_url(); ?>/dealer-services" style="color: #fff;"><strong>Dealership Services <span class="icon-forward"></span></strong></a></h3>

                        </div>
                    </div><!--[END .section-container]-->
                <?php endif; ?>

                <div id="login_or_register" class="login-btns">
                    <h2 class="login-title">Choose Option</h2>
                    <h2><button id="login_now" class="btn orange huge" style="width: 100%;">Login</button></h2>
                    <h2><button id="register_now" class="btn orange huge" style="width: 100%;">Register</button></h2>
                </div>
                <div id="register_seller" class="login-btns" style="display:none;">
                    <h2 class="login-title">Individual or Dealership?</h2>
                    <h2><button id="private" class="btn orange huge" style="width: 100%;">Private</button></h2>
                    <h2><button id="dealer" class="btn orange huge" style="width: 100%;">Dealer</button></h2>
                </div>
                ...
                <script>
                    jQuery(document).ready(function($) {
                        ...
                        $("#back_login_btn").click(function() {
                            ...
                            // $("#additional-settings").css("display", "block");
                        });
        ...
        public function additional_options() {
            ...
            // if ( ! isset( $wpml_settings['remove-reg'] ) ) {
                // if ( ( get_option( 'users_can_register' ) && ! is_multisite() ) || ( $multisite_reg == 'all' || $multisite_reg == 'blog' || $multisite_reg == 'user' ) )
                    // echo '<a href="#register" class="wpml-nav">' . __( 'Back', 'tdp-fec' ) . '</a> | ';
            // }
            echo '<a id="back_btn" href="" class="wpml-nav" style="display:none;">Back | </a>&nbsp;';
            echo '<a id="back_login_btn" href="" class="wpml-nav" style="display:none;">Back | </a>&nbsp;';
            echo '<a id="login_btn" href="#login" class="wpml-nav" style="display:none;">Login | </a>&nbsp;';
            echo '<a id="register_btn" href="" class="wpml-nav" style="display:none;">Register </a>&nbsp;';
            echo '<span id="pipe-break" style="display:none;"> | </span>';
            echo '<a id="password_btn" href="#forgotten" class="wpml-nav" style="display:none;">Lost/Forgot Password? </a>&nbsp;';
            // echo '<a href="#register" class="wpml-nav go-register">' . __( 'Register', 'tdp-fec' ) . '</a>';
            // echo '<a href="" class="wpml-nav log-to-reg" style="display:none;">Register</a>';
            // echo '<a href="#forgotten" class="wpml-nav lost-password">' . __( 'Lost your password?', 'tdp-fec' ) . '</a>';

            // echo '<div class="hide-login"> | <br /><a href="#login" class="wpml-nav">' . __( 'Back to Login', 'tdp-fec' ) . '</a></div>';
            // echo '<div class="hide-login"> | <a href="#login" class="wpml-nav">' . __( 'Login', 'tdp-fec' ) . '</a></div>';
            // echo '<div class="sfa-show-login" style="display:none;"> <a id="back" href="" class="wpml-nav">Back</a> | <a href="#login" class="wpml-nav go-login">' . __( 'Login', 'tdp-fec' ) . '</a></div>';
